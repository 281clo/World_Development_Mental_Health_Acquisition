...
def get_country_overview3(country, df1, df2, df3):
    df1 = df1.groupby(['Country','year']).mean()
    ...
    return df_1.mean(), df_3.mean(), df_7.mean(), mean([np.log(n) for n in df_2]), mean([np.log(n) for n in df_16]), df_18.mean(), df_15.mean(), df_4.mean(), df_5.mean(), df_6.mean(), df_8.mean(), df_19.mean(), df_20.mean(), df_21.mean()
